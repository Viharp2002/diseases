...
def display_information():
    ...
    with col1:
        ...
        thyroxine = st.text_input("Person on thyroxine medication? (yes/no)")
        sick = st.text_input("Person have been sick today? (yes/no)")
        pregnant = st.text_input("Is Person pregnant? (yes/no)")

    # Column 2
    with col2:
        thyroid_surgery = st.text_input("Person had Thyroid Surgery earlier? (yes/no)")
        tumor = st.text_input("Person has a tumor related to the thyroid? (yes/no)")
        hypopituitary = st.text_input("Person has hypopituitarism? (yes/no)")
        psych = st.text_input("Person suffers from psychological issues? (yes/no)")
        TSH_measured = st.text_input("TSH levels measured? (yes/no)")
    ...
    if st.button('Thyroid Test Result'):
        diagnosis = prediction_System([age, gender_value, thyroxine_value, sick_value, pregnant_value,
                                       thyroid_surgery_value, tumor_value, hypopituitary_value, psych_value,
                                       TSH_measured_value])
